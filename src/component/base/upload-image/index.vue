        ...
        <template v-for="(item, i) in itemList">
            <template v-if="item.display">
                <div class="thumb-item" :key="item.id" :style="boxStyle" v-loading="item.loading">
                    ...
                    <div class="control">
                        <i v-if="!disabled" class="el-icon-close del" @click.prevent.stop="delItem(item.id)"
                           title="删除"></i>
...
export default {
    ...
    computed: {
        ...
        src() {
            const {src} = this.configC
            return src
        },
        domain() {
            const {domain} = this.configC
            return domain
        },
        dataType() {
            const {dataType} = this.configC
            return dataType
        },
        rules() {
            const {rules} = this.configC
            return rules
        },
        /**
         * 上传图像数量下限
